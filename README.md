...
**v0.8.5 — stable.** See [CHANGELOG.md](CHANGELOG.md).
...
`social` adds two layers to any Smeldr application:
...
**Layer 1 — Agent routing:** Wire Smeldr lifecycle signals to outbound HTTP calls, so AI agents can react when content is published, scheduled, archived, or deleted.
...
`svc.Register(app)` wires REST endpoints auto-generated by Smeldr:
...
`AddRoutes` is one way to act on Smeldr signals. For in-process handlers (audit logs, cache invalidation, SSE), use `app.OnSignal()` directly — see the [Signal bus docs](https://smeldr.dev/docs/signal-bus).
...
```go
func verifySignature(body []byte, secret []byte, header string) bool {
